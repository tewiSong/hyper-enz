...
def glorot(tensor):
    if tensor is not None:
        stdv = math.sqrt(3.0 / (tensor.size(-2) + tensor.size(-1)))
...
class PMA(MessagePassing):
    ...
    def __init__(self, in_channels, hid_dim,
        ...
        self.lin_Q = Linear(in_channels, self.heads * self.hidden)
    ...
    def forward(self, x, edge_index: Adj,
        ...
        if isinstance(x, Tensor):
            ...
            x_Q = self.lin_Q(x).view(-1, H, C)
            x_K = self.lin_K(x).view(-1, H, C)
            x_V = self.lin_V(x).view(-1, H, C)
            alpha_r = (x_K * x_Q).sum(dim=-1)

        out = self.propagate(edge_index, x=x_V, alpha=alpha_r, aggrate=self.aggr)
    ...
    def aggregate(self, inputs, index,
        ...
        return scatter(inputs, index, dim=self.node_dim, reduce=aggrate)
